...
func TestUserHandler_GroupGetByID(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			ug.GroupID.String(),
			func(svc *mocks.MockUserService) {
				svc.EXPECT().GroupGetByID(gomock.Any(), userID, systemID, &groupID, nil).Return(nil, errors.ErrNotFound)
			},
			http.StatusNotFound,
			"",
		},
		{
			...
			ug.GroupID.String(),
			func(svc *mocks.MockUserService) {
				svc.EXPECT().GroupGetByID(gomock.Any(), userID, systemID, &groupID, nil).Return(ug, nil)
			},
			http.StatusOK,
			string(ugJSON),
		},
