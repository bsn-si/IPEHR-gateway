...
func TestQueryHandler_ExecGetQuery(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"4. invalid because no query in params",
			"ehr_id=7d44b88c-4199-4bad-97dc-d78268e01398&offset=1&fetch=10",
			func(svc *mocks.MockQueryService) {},
			400,
			`{"error":"Request validation error"}`,
		},
		{
			"5. error on get data",
			"ehr_id=7d44b88c-4199-4bad-97dc-d78268e01398&offset=1&fetch=10&q=SELECT%201",
			func(svc *mocks.MockQueryService) {
				r := &model.QueryRequest{
					Query:  "SELECT 1",
					Offset: 1,
		...
		{
			"6. success",
			"ehr_id=7d44b88c-4199-4bad-97dc-d78268e01398&offset=1&fetch=10&q=SELECT%201&some_val1=1&some_val2=some_str",
			func(svc *mocks.MockQueryService) {
				r := &model.QueryRequest{
					Query:  "SELECT 1",
					Offset: 1,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/v1/query/aql?%s", tt.queryParams), nil)
			req.Header.Set("Authorization", "Bearer AccessKey")
			req.Header.Set("AuthUserId", userID)
			req.Header.Set("EhrSystemId", systemID)
