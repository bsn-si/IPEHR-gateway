//
// description: ANTLR4 lexer grammar for Archetype Query Language (AQL) authors: Sebastian Iancu,
// Code24, Netherlands Teun van Hemert, Nedap, Netherlands Thomas Beale, Ars Semantica UK, openEHR
// Foundation Management Board contributors: This version of the grammar is a complete rewrite of
// previously published antlr3 grammar, based on current AQL specifications in combination with
// grammars of AQL implementations. The openEHR Foundation would like to recognise the following
// people for their contributions: - Chunlan Ma & Heath Frankel, Ocean Health Systems, Australia -
// Bostjan Lah, Better, Slovenia - Christian Chevalley, EHRBase, Germany - Michael Böckers, Nedap,
// Netherlands support: openEHR Specifications PR tracker
// <https://specifications.openehr.org/releases/QUERY/open_issues> copyright: Copyright (c) 2021-
// openEHR Foundation license: Creative Commons CC-BY-SA
...
//
// ======================= Lexical rules ======================== The followings are copies of
// https://github.com/openEHR/adl-antlr/blob/master/src/main/antlr/adl2/base_lexer.g4 rules, with
// some modifications required by AQL
// 
//
...
fragment TIMEZONE:
	'Z'
	| [+-] HOUR (':'? MINUTE)?; // hour offset, e.g. `+09:30`, or else literal `Z` indicating +0000.
fragment YEAR:
	[0-9][0-9][0-9][0-9]; // Year in ISO8601:2004 is 4 digits with 0-filling as needed
...
fragment ARCHETYPE_HRID_ROOT: (NAMESPACE '::')? IDENTIFIER_CHAR '-' IDENTIFIER_CHAR '-'
		IDENTIFIER_CHAR '.' ARCHETYPE_CONCEPT_ID;
fragment VERSION_ID:
	DIGIT+ ('.' DIGIT+)* (( '-rc' | '-alpha') ( '.' DIGIT+)?)?;
IDENTIFIER: IDENTIFIER_CHAR;
fragment IDENTIFIER_CHAR: ALPHA_CHAR WORD_CHAR*;
fragment ARCHETYPE_CONCEPT_ID: ALPHA_CHAR NAME_CHAR*;

// --------------------- composed primitive types ------------------- coded term shortcut e.g.
// 'ICD10AM(1998)::F23', 'ISO_639-1::en' or 'snomed_ct(3.1)::3415004|cyanosis|'
TERM_CODE:
...
URI:
	URI_SCHEME ':' URI_HIER_PART ('?' URI_QUERY)? (
		'#' URI_FRAGMENT
	)?;
...
fragment URI_SCHEME:
	ALPHA_CHAR (ALPHA_CHAR | DIGIT | '+' | '-' | '.')*;

fragment URI_AUTHORITY: (URI_USERINFO '@')? URI_HOST (
		':' URI_PORT
	)?;
fragment URI_USERINFO: (
		URI_UNRESERVED
		| URI_PCT_ENCODED
		| URI_SUB_DELIMS
		| ':'
...
fragment URI_HOST:
	URI_IP_LITERAL
	| URI_IPV4_ADDRESS
	| URI_REG_NAME; //TODO: ipv6
fragment URI_PORT: DIGIT*;

fragment URI_IP_LITERAL:
	'[' URI_IPV6_LITERAL ']'; //TODO, if needed: IPvFuture
fragment URI_IPV4_ADDRESS:
	URI_DEC_OCTET '.' URI_DEC_OCTET '.' URI_DEC_OCTET '.' URI_DEC_OCTET;
fragment URI_IPV6_LITERAL:
	HEX_QUAD (':' HEX_QUAD)* '::' HEX_QUAD (':' HEX_QUAD)*;

fragment URI_DEC_OCTET:
	DIGIT
	| [1-9] DIGIT
	| '1' DIGIT DIGIT
	| '2' [0-4] DIGIT
	| '25' [0-5];
fragment URI_REG_NAME: (
		URI_UNRESERVED
		| URI_PCT_ENCODED
		| URI_SUB_DELIMS
	)*;
...
fragment URI_PATH_ABSOLUTE:
	'/' (URI_SEGMENT_NZ ( '/' URI_SEGMENT)*)?;
fragment URI_PATH_NOSCHEME:
	URI_SEGMENT_NZ_NC ('/' URI_SEGMENT)*;
