...
func TestGetDocIndexByNearestTime(t *testing.T) {
	...
	if _, err := docService.GetDocIndexByNearestTime(ehrId, time.Now()); err == nil {
	...
	docIndex, err := docService.GetDocIndexByNearestTime(ehrId, lastDocIndexTime)
	...
	docIndex, err = docService.GetDocIndexByNearestTime(ehrId, DocIndexTimeMoreThanExist)
	if err != nil || docIndex == nil {
		t.Error("DocService not contains indexes")
	}
	...
	docIndex, err = docService.GetDocIndexByNearestTime(ehrId, DocIndexTimeLessThanExist)
	if err != nil || docIndex != nil {
